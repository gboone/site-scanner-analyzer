...
router.post('/import', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Pre-fetch existing domains to track inserted vs updated counts
    ...
    // Validate and pre-process — PostgreSQL aborts the whole transaction on error,
    // so filter out invalid rows before entering any transaction callback.
    const validRows: { row: Record<string, unknown>; isNew: boolean }[] = [];
    for (const s of allSites) {
      ...
      validRows.push({
        isNew: !existingDomains.has(domain),
        row: {
          domain,
          ...
          https: s.url ? (s.url.startsWith('https://') ? 1 : 0) : null,
      ...
      existingDomains.add(domain); // track within-payload dupes
    }
