...
export async function analyzeWpContent(baseUrl: string): Promise<WordPressContentResult> {
  ...
  for (const route of routes) {
    ...
    if (m && !builtinTypes.has(m[1])) customPostTypes.push(m[1]);
  }
  ...
  const postCount = postsRes.status === 'fulfilled' && postsRes.value.ok ? wpTotal(postsRes.value) : null;
  const pageCount = pagesRes.status === 'fulfilled' && pagesRes.value.ok ? wpTotal(pagesRes.value) : null;
  ...
  const categoryCount = catsRes.status === 'fulfilled' && catsRes.value.ok ? wpTotal(catsRes.value) : null;
  const tagCount = tagsRes.status === 'fulfilled' && tagsRes.value.ok ? wpTotal(tagsRes.value) : null;
